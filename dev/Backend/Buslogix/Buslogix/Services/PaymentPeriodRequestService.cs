...
namespace Buslogix.Services
{
    public class PaymentPeriodRequestService(IPaymentPeriodRequestRepository paymentPeriodRequestRepository) : IPaymentPeriodRequestService
    {
        ...
        {
            return await paymentPeriodRequestRepository.InsertPaymentPeriodRequest(companyId, request);
        }

        public async Task<bool> UpdatePaymentPeriodRequest(int companyId, int id, PaymentPeriodRequest request)
        {
            int affected = await paymentPeriodRequestRepository.UpdatePaymentPeriodRequest(companyId, id, request);
            return affected > 0;
        }

        public async Task<bool> DeletePaymentPeriodRequest(int companyId, int id)
        {
            int affected = await paymentPeriodRequestRepository.DeletePaymentPeriodRequest(companyId, id);
            return affected > 0;
        }
    }
}